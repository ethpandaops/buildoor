...
// to a JSON-RPC endpoint (typically buildoor's transaction pool ingress at
// /rpc). It prints the transaction hashes in submission order, one per line,
// so a caller can turn them into an exact per-slot plan (action plan
// build.local.queued).
//
//	go run ./.hack/txgen -rpc http://127.0.0.1:8086/rpc -keys k1,k2 -count 3
package main
...
func main() {
	rpcURL := flag.String("rpc", "http://127.0.0.1:8086/rpc", "JSON-RPC endpoint (buildoor pool ingress)")
	...
	for _, keyHex := range strings.Split(*keys, ",") {
		...
		// The ingress answers "pending" from the pool on top of the EL nonce.
