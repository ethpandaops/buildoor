...
// slotBuildState.started (the per-candidate build dedup map) is set before a
// build attempt; a failed attempt must clear it again, or a single transient
// engine error permanently blocks any retry of that (slot, parent-tuple)
// candidate for the rest of the slot — including a legitimate CL-client
// attributes redelivery for the exact same parent.
...
func TestExecuteCandidateBuild_RetriesAfterAFailedAttempt(t *testing.T) {
	...
	svc.payloadBuilder = NewPayloadBuilder(nil, nil, chainSvc, common.Address{}, cfgSvc, log, nil)
	...
	// arrives — real CL client behavior (reorgs, retries, some clients
	// simply re-emit payload_attributes). It must actually retry, not be
	// silently dropped because the tuple is still marked "started" from the
	// failed attempt.
	svc.executeCandidateBuild(slot, target)
	...
	case event := <-sub.Channel():
		require.Equal(t, slot, event.Slot)
	case <-time.After(time.Second):
		t.Fatal("second attempt for the same parent tuple never ran " +
