// Package validatorranges maps validator indices to client names using configured ranges.
...
// Resolver maps validator indices to client names.
...
// NewResolver creates a Resolver from config. Call Start to load ranges.
func NewResolver(cfg *config.ValidatorRangesConfig, log logrus.FieldLogger) *Resolver {
	return &Resolver{
		cfg: cfg,
		log: log.WithField("component", "validator-ranges"),
	}
}

// Start loads ranges immediately and, for URL sources, refreshes every 5 minutes.
...
// GetClientName returns the client name for a validator index, or "" if not found.
